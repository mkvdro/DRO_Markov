...
%         for k=1:d %compute partial derivative of (k-1)*pi_k wrt pm^i_j
            ...
            elseif i==d
                ...
                if i==k
                    ...
                    val0(i,j)=(base+(-1)^(i+j+d+k-1)*det(so_minor(A,d,j,min(k,i),max(k,i)))/det(A));
                end
            elseif j==d
                ...
                if i==k
                    val0(i,j)=base;
                elseif i<k
                    val0(i,j)=(base+(-1)^(d+k+1)*det(Mso)/det(A));
                else
                    val0(i,j)=(base+(-1)^(d+k)*det(Mso)/det(A));
                end
            else
                ...
                if i==k
                    val0(i,j)=base;
                elseif i<k
                   val0(i,j)=(base+((-1)^(d+k+1)*det(Mso)+(-1)^(d+k+i+j)*det(so_minor(A,d,j,min(k,i),max(k,i))))/det(A));
                else
                   val0(i,j)=(base+((-1)^(d+k)*det(Mso)+(-1)^(d+k+i+j+1)*det(so_minor(A,d,j,min(k,i),max(k,i))))/det(A)); 
                end 
            end
    end
end
